...
@if($model->status == 1 )
    <a href='{{auth()->user()->role == 'Super Admin' ? route("index_user", $model->id) : (auth()->user()->role == 'Kades' ? route("index_user-kades", $model->id) : route("index_user-kadus", $model->id)) }}' class="btn btn-outline-success btn-sm detail mr-2" title="Input Data Penerima"><i class="fa fa-user-circle-o"></i></a>
@endif
    ...
    <div class="modal fade" id="{{__('editpengguna'.$model->id)}}" tabindex="-1" role="dialog" aria-labelledby="defaultModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="edit_form" method="POST" action="{{route('edit_periode-kades', $model->id)}}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="customername-field" class="form-label">Nama Periode</label>
                            <input disabled type="text" value="{{$model->nama}}" name="edit_nama_periode" id="edit_nama_periode" class="form-control" placeholder="Masukkan Versi" required />
                        </div>
                        <div class="mb-3">
                            <label for="edit_period" class="form-label">Periode</label>
                            <input disabled type="text" name="edit_period" id="edit_period" class="form-control" value="{{date_format(date_create($model->periode), 'd-m-Y')}}" required />
                        </div>
                        <div class="mb-3">
                            <label for="kuota" class="form-label">Kuota</label>
                            <input disabled type="number" value="{{$model->kuota}}" min="1" name="edit_kuota" id="edit_kuota" class="form-control" placeholder="Masukkan Versi" required />
                        </div>
                        <div class="mb-3">
                            <label for="status" class="form-label">Status</label>
                            <select name="edit_status" class="form-control" id="edit_status">
    ...
    <div class="modal fade" id="{{__('editpengguna'.$model->id)}}" tabindex="-1" role="dialog" aria-labelledby="defaultModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="edit_form" method="POST" action="{{route('edit_periode-kadus', $model->id)}}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="customername-field" class="form-label">Nama Periode</label>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                @elseif(auth()->user()->role == 'Kades')
                    <a href="{{route("delete_periode-kades", $model->id)}}">
                        <button type="button" class="btn btn-danger">Hapus Data</button>
                    </a>
                @elseif(auth()->user()->role == 'Kadus')
                    <a href="{{route("delete_periode-kadus", $model->id)}}">
                        <button type="button" class="btn btn-danger">Hapus Data</button>
                    </a>
                @endif
